...
- 「この情報を削除する」ボタンを押すと **確認ダイアログ**（`confirm()` or モーダル）を表示
...
| # | 項目 | 備考 |
|---|---|---|
| 1 | 削除の確認UIは `window.confirm()` かモーダルか | 暫定: `window.confirm()` で実装、将来モーダル化 |
| 2 | 論理削除（`status='deleted'`）か物理削除（`DELETE`）か | 暫定: 物理削除。管理画面で復元が必要なら論理削除に変更 |
| 3 | ページネーションUIの共通化 | ジャンル一覧ページ（§19.2）と共通コンポーネントにするか |
