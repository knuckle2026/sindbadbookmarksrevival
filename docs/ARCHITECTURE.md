...
### 5.5 所在地絞り込み UI (地方 → 都道府県ナビ)

`hasPrefecture` ジャンルのページ上部に表示する所在地フィルタ
(`app/src/app/genres/[slug]/RegionPrefectureNav.tsx`)。状態は URL の
`region` / `prefecture` クエリで表し、リンク遷移でサーバ再描画する。

- **件数**: 各ボタンに `(件数)` を表示。件数 (`getPrefectureCounts` → `prefCountMap`)
  は **現在のカテゴリ絞り込みを反映**した都道府県別件数。地方ボタンの件数はその地方に属する
  都道府県件数の合計。
- **クイック選択**: 東京 / 大阪 / 名古屋 のチップ。選択中を再度押すと解除。
- **地方ボタン**: 全地方を常時表示。**選択中の地方も列に残し、赤くアクティブ表示 + 件数**を
  出す(選択中であることと絞り込み件数が一目で分かる)。**選択中の地方を再度押すと地方・
  都道府県の選択を解除**(トグル)。
- **都道府県**: 地方を選ぶと配下の都道府県ボタンを下に展開。県を選ぶとその県で絞り込み、
  選択中の県を再度押すと県のみ解除し地方選択は維持。
- 地方・都道府県をすべて未選択にすると所在地で絞らず**全件**(`prefecture=tokyo` への
  強制リダイレクトは無い)。
- 東京 / 大阪 を選んだ場合のみ、さらに区(ward)フィルタを別途展開する。
